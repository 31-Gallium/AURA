# skills/unit_converter_skill.py
import re
...
def convert_units(app, value, from_unit, to_unit, **kwargs):
    ...
    # Conversion factors dictionary (to meters or kilograms)
    factors = {
        'meters': 1.0, 'm': 1.0,
    ...
    if from_unit in factors and to_unit in factors:
        # Convert 'from_unit' to base unit, then to 'to_unit'
...
def register():
    """Registers the unit conversion command with a flexible regex."""
    return {
        'convert_units': {
            'handler': convert_units,
            'regex': r'\bconvert (\d+\.?\d*|\d+)\s(.+?)\s(?:to|in)\s(.+)\b',
            'params': ['value', 'from_unit', 'to_unit'],
            'description': "Converts a value between different units of measurement, like length, weight, or temperature."
        }
    }